...
@section('content')
<section id="page">
	{{Form::open(array('url' => 'admin/roles/'.$role->id, 'method' => 'put', 'data-abide' => 'data-abide'))}}
	<h1 class="columns">Редакция на роля <strong>{{$role->name}}</strong></h1>
	<div class="large-9 medium-8 medium-push-4 large-push-3 columns">
		<div class="row">
			<div class="medium-12 columns">
				{{Form::label('name', 'Име на роля')}}
				{{Form::text('name', $role->name, array('id'=>'name', 'placeholder'=>'Заглавие', 'required' => 'required'))}}
		...
		<div class="row">
			...
			@foreach($permissions as $permission)
				@if(in_array($permission->id, $role->perms()->lists('permission_id')))
					<?php $true=true;?>
				@else
					<?php $true=false;?>
				@endif
				<div class="medium-4 small-6 columns end">
					<label>{{Form::checkbox('permissions[]', $permission->id, $true)}} {{$permission->display_name}}</label>
				</div>
			@endforeach
		</div>
